...
| `SESSION_TTL_SECONDS` | | `300` | Сколько веб-сессия живёт после ухода последнего участника, секунды |

### Срок жизни веб-сессии

Веб-сессия живёт, пока к ней подключён хотя бы один участник. Когда уходит последний, засекается отсрочка `SESSION_TTL_SECONDS` (по умолчанию 300 секунд); по её истечении запись удаляется и из SQLite, и из памяти — комната перестаёт открываться по прямой ссылке, `GET /api/sessions/{id}` отвечает 404. Отсрочка нужна, чтобы обычная перезагрузка страницы не уничтожала раунд: она отключает всех разом.

Удаление выполняет фоновый уборщик, который просыпается раз в `SESSION_CLEANUP_INTERVAL`, поэтому фактическое время жизни осиротевшей сессии — от `SESSION_TTL_SECONDS` до `SESSION_TTL_SECONDS + SESSION_CLEANUP_INTERVAL`, то есть от 5 до 15 минут при значениях по умолчанию. На публичном стенде интервал разумно снизить до 60, чтобы данные задач не залёживались.

Правило касается только веб-сессий. Игры, созданные командой `/poker` в Telegram, хранятся в той же таблице, но WebSocket-подключений у них нет вообще, и уборщик их не трогает.
...
1. Скачайте расширение: [Скачать расширение](http://localhost:8000/extension/download) — архив собирается под ваш браузер
2. Распакуйте ZIP в **новую пустую** папку (старую сотрите: иначе легко загрузить прошлую сборку)
3. Откройте `chrome://extensions`, для Firefox — `about:debugging#/runtime/this-firefox`
4. Включите **"Режим разработчика"** (в Firefox не нужно)
5. Нажмите **"Загрузить распакованное расширение"** (в Firefox — "Временная загрузка дополнения")
6. Выберите папку с расширением (в Firefox — файл `manifest.json` в ней)
7. Обновите страницу Planning Poker

Подробные инструкции для конкретного браузера открываются по адресу
`/extension/download?download=html`.

### Настройка

1. Откройте popup расширения и впишите адрес Planning Poker в поле
   **«Адреса Planning Poker»** — по одному в строке, только схема и хост
   (например `http://localhost:8000`). Со страниц, которых нет в этом списке,
   расширение не работает: иначе любой открытый сайт мог бы запросить у него
   сохранённый токен Jira. Нажмите **"Сохранить настройки"**.
2. Откройте панель **⚡ JIRA** в правом верхнем углу
3. Введите URL вашей Jira
4. Введите API Token (Personal Access Token)
5. Настройте JQL-фильтр для загрузки задач
6. Нажмите **"СОХРАНИТЬ"**
